...
<body>
    ...
    <div class="section-header">
     ...
     <div id="lansia-ujungpadang-tab" class="lansia-tab">
        <div class="table-responsive">
            <table class="table table-striped table-hover">
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php
                    $no = 1;
                    $query = mysqli_query($conn, "SELECT * FROM lansia_rian ORDER BY id DESC");
                    while ($row = mysqli_fetch_assoc($query)):
                    ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            ...
                            <td>
                                <span class="badge badge-<?=
                                    $row['status_kesehatan'] == 'Sehat' ? 'success' : 
                                    ($row['status_kesehatan'] == 'Kurang Sehat' ? 'warning' : 
                                    ($row['status_kesehatan'] == 'Sakit' ? 'danger' : 
                                    ($row['status_kesehatan'] == 'Dalam Perawatan' ? 'info' : 'secondary'))) ?>">
                                    <?= $row['status_kesehatan'] ?: 'Belum diperiksa' ?>
                                </span>
                            </td>
                            <td>
                                <a href="lansia_edit.php?id=<?= $row['id'] ?>&kader=rian" class="btn btn-sm btn-primary">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="lansia_delete.php?id=<?= $row['id'] ?>&kader=rian" class="btn btn-sm btn-danger" onclick="return confirm('Yakin ingin menghapus?')">
                                    <i class="fas fa-trash"></i>
                                </a>
                            </td>
                        </tr>
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    <?php
                    $no = 1;
                    $query = mysqli_query($conn, "SELECT * FROM lansia_yanti ORDER BY id DESC");
                    while ($row = mysqli_fetch_assoc($query)):
                    ?>
                        <tr>
                            <td><?= $no++ ?></td>
...
<script >
    function showLansiaTab(desa, event) {
    ...
    document.querySelectorAll('#data_lansia .btn-group .btn').forEach(btn => {
        btn.classList.remove('active');
    });

    // Tampilkan tab yang dipilih
    const tabId = 'lansia-' + desa + '-tab';
...
// Initialize with first tab shown by default
document.addEventListener('DOMContentLoaded', function() {
    showLansiaTab('ujungpadang');
});
</script>
