    ...
    <body>
        <div class="access-denied-container">
            ...
            <div class="error-message">
                ...
                <strong>Halaman Edit Data Anak</strong> ini memiliki akses terbatas dan hanya dapat digunakan oleh <strong>Kader bernama maulana</strong> untuk menjaga keamanan dan integritas data.
            </div>
...
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'kader' || $_SESSION['username'] !== 'Maulana') {
    showAccessDenied();
}
...
// PERBAIKAN: Pastikan nama tabel konsisten - gunakan anak_maulana
$stmt = $conn->prepare("SELECT * FROM anak_maulana WHERE id = ?");
...
if (!$data) {
    echo "<script>
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({
                ...
                text: 'Data anak yang Anda cari tidak tersedia.',
    ...
    // Jangan exit di sini, biarkan halaman load untuk menampilkan SweetAlert
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $alamat = "Sialang"; // Desa tetap
    $umur = hitungUmurBulan($tgl_lahir); // Hitung umur dalam bulan
    
    // Update data
    $stmt = $conn->prepare("UPDATE anak_maulana SET 
        nama = ?, jk = ?, tgl_lahir = ?, umur = ?, orang_tua = ?, alamat = ?,
        bb_lalu = ?, bb_ini = ?, pbtb_lalu = ?, pbtb_ini = ?, lk_lalu = ?, lk_ini = ?,
        status_gizi = ?, ket = ?
        WHERE id = ?");
...
<html>
<head>
    <title>Edit Data Anak - Sialang</title>
...
<div class="container">
    <div class="header">
        <h2><i class="fas fa-edit"></i> Edit Data Anak</h2>
        <p>Desa Sialang - Sistem Informasi Balita</p>
    </div>
    ...
    <form method="POST">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" step="0.1" name="bb_lalu" value="<?= $data['bb_lalu']; ?>">
            </div>
            <div class="form-group">
                <label><i class="fas fa-weight"></i> Berat Badan Sekarang (kg)</label>
                <input type="number" step="0.1" name="bb_ini" value="<?= $data['bb_ini']; ?>">
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" step="0.1" name="pb_lalu" value="<?= $data['pbtb_lalu']; ?>">
            </div>
            <div class="form-group">
                <label><i class="fas fa-ruler-vertical"></i> Panjang/Tinggi Badan Sekarang (cm)</label>
                <input type="number" step="0.1" name="pb_ini" value="<?= $data['pbtb_ini']; ?>">
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" step="0.1" name="lk_lalu" value="<?= $data['lk_lalu']; ?>">
            </div>
            <div class="form-group">
                <label><i class="fas fa-circle"></i> Lingkar Kepala Sekarang (cm)</label>
                <input type="number" step="0.1" name="lk_ini" value="<?= $data['lk_ini']; ?>">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="keterangan" rows="4"><?= htmlspecialchars($data['ket']); ?></textarea>
        </div>
